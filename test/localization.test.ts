...
test("every literal runtime localization key is in the English bundle",()=>{const bundle=JSON.parse(readFileSync("l10n/bundle.l10n.json","utf8")) as Record<string,string>;const files=readdirSync("src",{recursive:true}).map(String).filter(file=>file.endsWith(".ts")).map(file=>`src/${file}`);for(const file of files){const source=readFileSync(file,"utf8");for(const match of source.matchAll(/vscode\.l10n\.t\("([^"]+)"/g)){const key=match[1]!.replaceAll("\\n","\n");assert.ok(bundle[key],`${file}: missing ${match[1]}`);}assert.doesNotMatch(source,/new\s+[A-Za-z]*Error\s*\(\s*[`'"]/,`${file}: literal error bypasses localization`);}});
